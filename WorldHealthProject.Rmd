...
The focus of project one for team Wasted Potential was to develop and answer a research-driven SMART question on a health-related dataset. Although we found the dataset on Kaggle, it is originally sourced from the Global Health Observatory (GHO) which is a data repository owned by the World Health Organization (WHO).  The dataset contains 2,938 observations spread over 22 variables that contribute to the overall health status of 193 countries. Our SMART question was to determine which of the given variables exhibits a measurable effect on the life expectancy of different countries. Explanations for what each of the variables represents is displayed below. [INSERT FIGURE]. It is important to note that the economic data for this dataset (i.e. total expenditure, GDP) was sourced from the United Nations website, as noted on Kaggle. As the author of the Kaggle webpage for the dataset writes, “It has been observed that in the past 15 years, there has been a huge development in health sector resulting in improvement of human mortality rates especially in the developing nations in comparison to the past 30 years. Therefore, in this project we have considered data from year 2000-2015 for 193 countries for further analysis” (Kumar Rajarshi, 2018). As explained later in this paper, there were a variety of missing and likely incorrect values for different countries. The dataset on Kaggle excludes lesser-known countries like Tonga, Togo and Cabo Verde, whose data was almost all missing or inaccurate. 
For visualization, we constructed a world map to display the distribution of life expectancies across the world. Our world map graphic helps visualize the difference in life expectancy between developed and developing countries, as well as how life expectancy changes in countries over time. Our preprocessing for the world map consisted of adding levels and changing the names for different countries in our dataset to match what was present in ggplot2’s map_data. As explained on RDocumentation, the purpose of map_data is to “Easily turn data from the maps package in to a data frame suitable for plotting with ggplot2” (Hadley Wickham, n.d.). The following code was written to organize all existing countries and their data into a data frame:
world_data <- map_data('world')
For example, in our dataset the United States is titled “United States of America,” but in world_data it is titled “USA.” The purpose for changing the country names is to successfully merge the two datasets together. World_data contains columns for the respective latitude-longitude coordinates of all countries, which is used to identify where to graph countries on the world map. In our data set, there are no latitude-longitude coordinates provided. Thus, by merging the datasets together using the country column, the result will be a dataset containing all health-related data and latitude-longitude coordinates for countries of interest. However, by using the match function, we are effectively performing an inner join. As RDocumentation explains, “match returns a vector of the positions of (first) matches of its first argument in its second” (R-core R-core@R-project.org, n.d.). Thus, by using the country name as the column to merge from, only countries with exact name matches will be merged. By not changing the names of countries in our original dataset, they will not be merged with world_data and the world map produced will have no data for countries with mismatched names. 
Once levels were added to account for eighteen different country name mismatches, the world map was constructed using ggplot2. It is important to note that to display the life expectancy to visualize the difference between developed and developing countries, the mean life expectancy across all years for each country (2000-2015) was computed and displayed. For the graphic that displays trends in life expectancy changes across the years, every four years between 2000 and 2015 was graphed (did not want to print out sixteen different graphs and overload presentation). The result of this world map visualization is that the gap in average life expectancy between developed and developing countries is about ten to fifteen years, which is astounding. This data reassured us that the status of a country has a profound correlation with its life expectancy. However, the status of a country carries several other factors with it (i.e. GDP, population and disease levels) which are likely the more measurable contributors to the value of life expectancy.  

#### ______________________________________________________________________________



Life expectancy is a summary of the overall mortality level of a given population. The reported life expectancy reflects that mortality pattern that exists across all age groups in the current year. In 2016, the global life expectancy was 72 years old, with female values reporting roughly 4 years longer than males. What was particularly interesting was the wide range of values by location that encompassed that overall 72 years old value. In that year the reported values ranged from 61.2 years old in the WHO African region to 77.5 years old in the WHO European Region (taken from statistics on who.int site), a gap that is about 16 years in size. These values highlight the huge inequality in how health is distributed around the world. It pushed us to question what differing factors could have affected the trends in mortality so drastically across the globe? We found a public dataset from Kaggle ("Life Expectancy (WHO): Statistical Analysis on factors influencing Life Expectancy") that provided 2,938 observations across 22 variables. The variables ranges from health factors such as BMI and immunizations to population factors such as developing status and GDP (Gross Domestic Product per capita). The data was collected from the World Health Organization (WHO) website and the United Nations website and then posted on Kaggle where we discovered it. We utilized a combination of decriptive graphs and tests to explore our data and given variables. Our Exploratory Data Analysis will be useful in identifying key factors that can aid in building models to predict future life expectancy values.  

#### ______________________________________________________________________________

## Ben's Part: Introduction, explanation of dataset and world map visualization explanation (current word count: **715**)

## RyeAnne's Part: Correlation, Significance of Correlated Variables, Association of Life Expectancy with Factor Variables, Differences in Variables Between Developed and Developing Countries, Part of the Conclusion (current word count: **940**)

## [__________]'s Part: [______] (current word count: **_____**)
...
note --> should this be a subheading? idk where to put it 

As we were beginning our analysis of the dataset, we noticed some unusual values in some graphs and statistics. For instance, the "percentage expenditure" variable is a reflection of the money spent on health as a percentage of GDP per capita, as such, the highest possible number a percent value can be in this context is 100%. However, we noticed that one of the data points showed 19480 as the percentage expediture for Switzerland which did not make sense. In addition, there was also a datapoint thatt showed China's percentage expediture as zero, which we also know is not true. In other instances such as for population values, we notices values in the thousands, when they should have been in the millions range. 

---> going to talk about more problems, etc. 

We believe that some of the discrepancies in the dataset were perhaps due to oweners hand entering values or improperly joining the data from the two different sources (from the WHO and the UN). 
...
The scatterplot below, "Per Capita Health Expediture (y) vs GDP (x)", shows a positive relationship between life expectancy and the amoutn of money spent on healthcare as a % of GDP per capita. This is more in line with our hypothesis that the more money spent on healthcare should result in better health outcomes and higher life expectancy ages. It seems that the difference between the graph below and the graph above is indicating that different amounts of government spending does not have a great effect but the overall amount that citizens are willing to spend will be more indicative of greater life expectancy. One hypothesis is that no matter how much the government is willing to spend or not spend on their citizens, each person will do their best to supplement the amount given to them with the sufficient fundign to meet ttheir healthcare needs, this could be studied further with more quantitative spending data instead of just percentages.  
...
The scatterplot below, "Life Expectancy (y) vs Deaths due to HIV/AIDS (x)", shows a
...
The scatterplot below, "Life Expectancy (y) vs Years of Schooling (x)", shows a
