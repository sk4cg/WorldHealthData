...
status_ttest = t.test(life_developed$Life.expectancy, life_developing$Life.expectancy, alternative = "two.sided", var.equal = FALSE) #t-test to determine if the life expectancy of the two groups is different
status_ttest

#make overlaying histograms of the two subgroups
library(ggplot2)
library(dplyr)
life_hist=ggplot(life,aes(x=Life.expectancy)) + 
    geom_histogram(data=subset(life,Status == 'Developed'),fill = "red", alpha = 0.4) +
    geom_histogram(data=subset(life,Status == 'Developing'),fill = "blue", alpha = 0.4) +
    labs(x="Life Expectancy")+
    ggtitle("Life Expectancy of Developed vs Developing Countries") +
